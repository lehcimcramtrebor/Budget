...
| **Bannière Reste à Vivre** | `id="main_banner"` | Affiche le solde global restant, les revenus cumulés et la tirelire Cochon. |
...
*   `id="cochon_modal"` : Fenêtre interactive pour alimenter ou casser sa tirelire Cochon.
...
*   `calculateSmartRounding(amount)` : Détermine le pourboire optionnel à verser à la tirelire Cochon en arrondissant la dépense à l'euro supérieur.
...
*   `testCochonLogic()` : Valide le versement des pourboires et le comportement de la tirelire Cochon.

---

## 🔗 4. Le pont de compatibilité globale (`window`)

Comme `app.js` est chargé sous forme de module ES6 (`type="module"`), ses fonctions ne sont plus accessibles dans la portée globale par défaut. Pour éviter de casser les attributs événementiels inline du HTML (comme `onclick="openCochonModal()"`), toutes les fonctions requises sont explicitement exposées sur l'objet global `window` en fin de fichier :
